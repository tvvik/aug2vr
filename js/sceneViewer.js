var ViewerScene = function() {
    ...
    var onDeviceMotion = function(event) {
        ...
        _speed.y = _acceleration_multiply*az;

        var position = this.camera.position.clone();
        var target = new Vector3(_speed.x, _speed.y, 0);
        //console.log('SPEEEED: ', _speed);
        this.camera.position.x += _speed.x;
        this.camera.position.y += _speed.y;
    ...
    var slider1Change = function(event) {
        //console.log('value: ', event.target.value);
        //console.log('camera: ', this.camera)
        this.camera.fov = event.target.value;
	...
	this.initialise = function(container)
	{
        ...
        var sphGeom = new THREE.SphereGeometry(100, 60, 40);
        sphGeom.scale(1,1,1);
        
        this.markerObject3D = new THREE.Mesh(sphGeom, new THREE.MeshStandardMaterial({color:"#333333"}));

        // this.sphere2 = new THREE.Mesh(sphGeom, this.materialRight);

        // this.effect = new THREE.StereoEffect(this.renderer, this.sphere1, this.sphere2);
        ...
        this.scene.add(this.markerObject3D);
    ...
    this.initMarkers = function() {
        ...
        document.getElementById('aug').appendChild(videoGrabbing.domElement);
    }
	...
	this.loadTextures = function() {
        ...
        loaderTexR.load('images/stereo/003/right.jpg',
            function (txx){
                txx.wrapS = THREE.RepeatWrapping;
                txx.wrapT = THREE.RepeatWrapping;
    ...
    this.addSliderEvent = function() {
        document.getElementById('slider1').addEventListener('input', slider1Change);
        document.getElementById('slider1').addEventListener('change', slider1Change);
    }

	this.animate = function() {
        if (videoGrabbing) checkForMarkers();
    ...
    function checkForMarkers(){
        //console.log('videoGrabbing: ',videoGrabbing);
        ...
        if (this.markerObject3D) this.markerObject3D.visible = false
        // see if this.markerId has been found
        markers.forEach(function(marker){
            // if( marker.id !== 265 )  return
            jsArucoMarker.markerToObject3D(marker, this.markerObject3D)
            if (this.markerObject3D) this.markerObject3D.visible = true
        })
    }
